...
class App(customtkinter.CTk):
    ...
    def btn_mostrar_on_click(self):
        ...
        if edad < 16:
            if estado == "NATIVO":
                mensaje = "{0} {1}".format(votar_no, menor_nac)
                # mensaje = f"{votar_no} {menor_nac}"
            else:
                mensaje = "{0} {1}".format(votar_no, menor_nat)
                # mensaje = f"{votar_no} {menor_nat}"
        if edad >= 16 and edad <18:
            if estado == "NATIVO":
                mensaje = "{0}".format(votar_si)
                # mensaje = f"{votar_si}"
            else:
                mensaje = "{0} {1}".format(votar_no, menor_nat)
                # mensaje = f"{votar_no} {menor_nat}"
